...
/**
 * PRO upgrade promotion, shown ONLY on the Estimate settings screen: a dismissible
...
{
    private const META   = 'estimate_pro_banner_dismissed';
    private const ACTION = 'estimate_dismiss_pro';
    ...
    {
        if ($this->data === null) {
            $file = ESTIMATE_DIR . 'config/pro-upsell.php';
    ...
    public function enabled(): bool
    {
        ...
        if (defined('Estimate\\Pro\\VERSION')) {
            return false;
        }
        /**
         * Filters whether the Estimate PRO promo is shown on the settings screen.
         *
         * @param bool $show Default true.
         */
        return (bool) apply_filters('estimate/show_pro_cta', true) && $this->features() !== [];
    }

    private function url(): string
    {
        $default = (string) ($this->data()['url'] ?? 'https://plogins.com/plogins-estimate-pro/pricing/');
        /**
         * Filters the URL the PRO call-to-action buttons point at.
         *
         * @param string $url Default the Estimate PRO page.
         */
        return (string) apply_filters('estimate/pro_url', $default);
    }
    ...
    private function priceLabel(): string
    {
        ...
        if (! empty($d['price_from'])) {
            ...
            return sprintf(__('from %1$s%2$d/yr', 'quotlet'), $cur, (int) $d['price_from']);
        }
        return '';
    }
    ...
    public function handleDismiss(): void
    {
        if (! current_user_can('manage_woocommerce')) {
            wp_die(esc_html__('Permission denied.', 'quotlet'));
        }
        check_admin_referer(self::ACTION);
        update_user_meta(get_current_user_id(), self::META, 1);
        wp_safe_redirect(wp_get_referer() ?: admin_url('admin.php?page=estimate-settings'));
        exit;
    }
    ...
    public function banner(): void
    {
        ...
        $name     = (string) ($this->data()['name'] ?? 'Quotlet Pro');
        ...
        ?>
        <div class="estimate-pro-banner" role="note">
            <span class="estimate-pro-banner__tag">PRO</span>
            <p class="estimate-pro-banner__text">
                <strong><?php
                /* translators: %s: PRO edition name */
                printf(esc_html__('Do more with %s', 'quotlet'), esc_html($name)); ?></strong>
            ...
            <a class="estimate-pro-banner__dismiss" href="<?php echo esc_url($this->dismissUrl()); ?>" aria-label="<?php esc_attr_e('Dismiss this notice', 'quotlet'); ?>">&times;</a>
        </div>
        <?php
    }
    ...
    public function aside(): void
    {
        ...
        <aside class="estimate-card estimate-pro-aside" aria-labelledby="estimate-pro-aside-h">
            ...
            <h2 id="estimate-pro-aside-h" class="estimate-pro-aside__heading"><?php esc_html_e('Unlock every PRO feature', 'quotlet'); ?></h2>
            <ul class="estimate-pro-aside__list">
                <?php foreach ($features as $f) : ?>
                    <li>
                        <span class="estimate-pro-aside__lock" aria-hidden="true"></span>
                        <span><?php echo esc_html($f['title']); ?></span>
                    </li>
                <?php endforeach; ?>
            </ul>
            <a class="button button-primary button-hero estimate-pro-aside__cta" href="<?php echo esc_url($this->url()); ?>" target="_blank" rel="noopener noreferrer">
                <?php echo esc_html($this->ctaLabel()); ?>
            </a>
            <?php if ($price !== '') : ?>
                <p class="estimate-pro-aside__price"><?php echo esc_html($price); ?><?php if ($this->sellable()) : ?> · <?php esc_html_e('one licence, every PRO feature', 'quotlet'); ?><?php endif; ?></p>
            <?php endif; ?>
        </aside>
        <?php
    }
    ...
    public function cards(): void
    {
        ...
        <section class="estimate-pro-cards" aria-labelledby="estimate-pro-cards-h">
            <h2 id="estimate-pro-cards-h" class="estimate-pro-cards__title">
                <?php
                /* translators: %s: PRO edition name */
                printf(esc_html__('What %s adds', 'quotlet'), esc_html($name)); ?>
            </h2>
            <div class="estimate-pro-cards__grid">
                <?php foreach ($features as $f) : ?>
                    <article class="estimate-pro-card">
                        <span class="estimate-pro-card__badge">PRO</span>
                        <span class="estimate-pro-card__lock" aria-hidden="true"></span>
                        <h3 class="estimate-pro-card__title"><?php echo esc_html($f['title']); ?></h3>
                        <?php if ($f['desc'] !== '') : ?>
                            <p class="estimate-pro-card__desc"><?php echo esc_html($f['desc']); ?></p>
                        <?php endif; ?>
                    </article>
                <?php endforeach; ?>
            </div>
